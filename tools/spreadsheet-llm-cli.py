...
Environment Variables:
  ...
  OPENAI_BASE_URL     Custom OpenAI-compatible API endpoint (e.g., for local models)
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-4o",
        help="LLM model to use for recognition (default: gpt-4o). Examples: gpt-4o, gpt-3.5-turbo",
    )
    ...
    if wb := wrapper.read_spreadsheet(file):
        ...
        if result := wrapper.compress_spreadsheet(
            ...
            # Add sheet name to filename to distinguish different sheets
            sheet_suffix = f"_{result.sheet_name}" if result.sheet_name else ""
            suffix = sheet_suffix + ("_format_aware" if args.format_aware else "")
